...
from kermt.data.molfeaturegenerator import get_features_generator
...
class MoleculeDataset(Dataset):
    ...
    def features_size(self) -> int:
        ...
        """
        onthefly_feature_size = 0
        if self.data[0].features_generator is not None:
            for fg in self.data[0].features_generator:
                if fg.endswith('_onthefly') or fg.endswith('_cuik_molmaker'):
                    onthefly_feature_size += 200 # Size of RDKit2DNormalized features.
        else:
            onthefly_feature_size = 0
        
        return (len(self.data[0].features)+onthefly_feature_size) if len(self.data) > 0 and self.data[0].features is not None else 0
